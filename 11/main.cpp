...
void explodeOcto(std::vector<short *> &data, size_t len,
                 std::stack<std::pair<int, int>> &coords, int i,
                 int j) {
...
void explode(std::vector<short *> &data, size_t len,
  ...
  while (coords.size() > 0) {
    ...
    explodeOcto(data, len, coords, i, j);
  }
}

void explodeSync(std::vector<short *> &data, size_t len,
                 std::stack<std::pair<int, int>> &coords, bool &allExp,
                 size_t &dataSize) {
  int i, j;
  auto addIfNotZero = [](short &d) {
    if (d != 0)
...
int main() {
  ...
  std::vector<short *> dataCopy;
  deepCopy(data, dataCopy, len);
  ...
  std::cout << len << std::endl;
    ...
    for (auto i = 0; i < count; i++) {
      increaseEnergy(dataCopy, len, explodeCoords);
      explode(dataCopy, len, explodeCoords, explodeCount);
    }

  std::cout << explodeCount << std::endl;
  count = 0;
